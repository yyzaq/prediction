...
SEMI_DISCRETE_CONFIG = {
    ...
    "phase_configs": [
        # 阶段1：宽松环境
        {
            ...
            "training": {"timesteps": 100_000, "learning_rate": 3e-4}
        },
        # 阶段2：中等环境
        {
            "name": "phase2_medium",
            ...
            "training": {"timesteps": 60_000, "learning_rate": 2e-4}
        },
        # 阶段3：严格环境
        {
            "name": "phase3_hard",
            ...
            "training": {"timesteps": 40_000, "learning_rate": 1e-4}
        }
    ]
}
...
class CourseLearningTrainer:
    ...
    def _create_model(self, env, phase_config: Dict) -> PPO:
        ...
        gamma=0.95,  # 降低折扣因子，更关注近期回报
        n_steps=1024,  # 减少步数，更快学习
        batch_size=128,  # 减小批大小，更快更新
        n_epochs=5,  # 减少epoch数
        clip_range=0.3,  # 增大clip范围，允许更大更新
        verbose=1,
        seed=self.config["SEED"],
        policy_kwargs=dict(
            net_arch=[128, 128],  # 简化网络
            activation_fn=torch.nn.ReLU,  # 使用ReLU增强非线性
        ),
